...
int main(int argc, char *argv[]) {
	...
	*errorCode = 1;
    ...
    temp = chrono::high_resolution_clock::now();
    conflictDetected = true;
    while (conflictDetected) { // run kernel until no conflict occurs
		...
		*errorCode = 0;
        ...
        checkCudaError(cudaError, "memcpy results - for cpu");
        detectConflictsCPU(row_ptr, col_ind, nov, results);
        ...
        cudaError = cudaMemcpy(d_results, results, nov * sizeof(uint), cudaMemcpyHostToDevice);
        checkCudaError(cudaError, "memcpy d_results - back to gpu");
